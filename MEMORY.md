# VersionCheck Memory 🧠

## v0.2.4 (2026-09-18)
- **Silent Missing Apps**: Неустановленные утилиты теперь автоматически и тихо пропускаются через `shutil.which` и `FileNotFoundError` без вывода красных сообщений `❌ Command not found`. Вывод консоли остаётся кристально чистым на любом девайсе (отображаются только реально установленные программы).

## v0.2.3 (2026-09-18)
- **GitHub Token Auto-Detection**: Автоматическое использование `GITHUB_TOKEN`/`GH_TOKEN` или токена из `gh auth token` для увеличения лимита запросов GitHub API с 60 до 5000 в час (защита от HTTP 403 Rate Limit).

## v0.2.2 (2026-09-18)
- **LocalSend**: Добавлена кроссплатформенная поддержка LocalSend (`localsend/localsend`) для Linux и macOS. Локальная версия считывается напрямую из `version.json` (Linux) или `Info.plist` (macOS). Встроен монолитный кроссплатформенный апдейтер прямо в `check_versions.py` (без внешних shell-скриптов): поддержка tar.gz/chmod на Linux, Homebrew Cask / DMG mount на macOS.

## v0.2.1 (2026-07-29)
- **Ignored Status Visuals**: Заменен эмодзи `✔️` на `⏸️` (Pause) для пропущенных/игнорируемых обновлений (`ignore_update`), чтобы визуально отличать актуальное состояние от отложенного обновления.

## v0.2.0 (2026-07-20)
- **Grok Build CLI (`grok`)**: Добавлена проверка версии Grok через официальный API `https://x.ai/cli/stable` и обновление через официальный install-скрипт (`curl -fsSL https://x.ai/cli/install.sh | bash`).

## v0.1.8 (2026-05-24)
- **Antigravity CLI (`agy`)**: Добавлена проверка версий через официальный эндпоинт авто-апдейтера и автоматическое обновление через `agy update`.
- **Antigravity IDE (`agy-ide`)**: Интегрирована проверка версии. Локальная версия считывается из файла `product.json` (`ideVersion`), удаленная — с угаданного нами API авто-апдейтера. Выставлен режим `ignore_update=True` для ручного обновления на Linux.
- **OpenCode**: Добавлен мониторинг версий из репозитория `anomalyco/opencode` с поддержкой автообновления через `opencode upgrade`.
- **Универсальные URL для версий (`version_url`)**: Внедрена поддержка произвольных веб-эндпоинтов для проверки актуальных версий софта в обход GitHub Releases.
- **Dynamic Paths**: Скрипт переведен на динамическое определение собственного пути через `Path(__file__).resolve()`, что делает его полностью независимым от текущего рабочего каталога запуска.
- **Aesthetics & Alignment**: Вся сетка вывода терминала расширена с 12 до 15 символов ширины имени для идеального вертикального выравнивания двоеточий.